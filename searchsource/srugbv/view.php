...
/**
 * The script to display and process the configuration form for searchsources of type "srugbv"
 *
...
$url = new moodle_url('/mod/literature/searchsource/srugbv/view.php');
$url->param('id', $id);
$PAGE->set_url($url);
...
$form = new literature_searchsource_srugbv_form();
...
if ($form->is_cancelled()) {
    ...
    $data = $_POST;
    $data['id'] = $id;
    list($defaultvalues, $url) = literature_searchsource_srugbv_processform($data);
    ...
    $defaultvalues->bibcode = $source->bibcode;

    // Set page data
    $PAGE->set_title(get_string('editsource', 'searchsource_srugbv'));
    $PAGE->set_heading($source->name);
} else {

    // Set page data
    $PAGE->set_title(get_string('editsource', 'searchsource_srugbv'));
    $PAGE->set_heading(get_string('newsource', 'searchsource_srugbv'));
}
...
$form = new literature_searchsource_srugbv_form('view.php?id=' . $id);
